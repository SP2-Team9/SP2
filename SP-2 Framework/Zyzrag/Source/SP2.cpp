...
void SP2::objectsInit()
{
	...
	//Vehicles Init
    ship = new Vehicles(Vector3(100, 0, -100), Vector3(0, 0, 1), 20, 50, 500, 30);
	ship->SetHitboxSize(5);
    ship->SetInteractionSize(10, 10, 10, 10, 10, 10);
    ship->initialMoveDirection();

  /*  boat = new Vehicles(Vector3(0, 0, -100), Vector3(1, 0, 0), 30, 50, 50, 30);
...
void SP2::checkHitboxes()
{
	...
	case CheckShips:
		// Ship to Ship
		for (vector<Vehicles*>::iterator V1it = allVehicles.begin(); V1it != allVehicles.end();)
		{
			Vehicles* Veh1 = *V1it;
			for (vector<Vehicles*>::iterator V2it = allVehicles.begin(); V2it != allVehicles.end();)
			{
				Vehicles* Veh2 = *V2it;
